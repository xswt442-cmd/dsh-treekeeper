...
- 「DSH 宿主后代」改为默认收起的折叠区，与其他分区一致。这些进程已计入顶部概览，展开的清单会把可操作分区（未归属进程、任务账本、子代理树）挤到首屏之外。数量角标仍在收起的标题上可见。
- README 抬头改用一致的徽章行：npm 版本、下载量、DSH 兼容范围、仅 Windows 的平台声明、Node 版本与许可证。
