...
import { VERSION, sendJson, requirePost, treekeeperGuard, pluginHint, subagentAvailability, resolveDshHome } from './shared.js'
    ...
    const apiRoute = {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (action === 'history') {
            sendJson(res, 200, { ok: true, rows: await history.last(100) })
            return
          }
          if (action === 'config') {
          ...
          if (action === 'kill') {
            ...
            const identity = decideKillConfirm({ pid, seenCreatedMs, fresh: await sample() })
            if (!identity.ok) {
