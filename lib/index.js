...
import { createJobLedger, listSubagentTree } from './ledger.js'
      ...
      handler: async (req, res) => {
        try {
          ...
          if (action === 'snapshot') {
            ...
            const subagents = rootSessionId === null ? [] : await listSubagentTree(ctx, rootSessionId)
            ...
            sendJson(res, 200, {
              ...
              subagentAvailability: rootSessionId === null
                ? 'root-required'
                : (subagents === null ? 'unavailable' : 'available'),
              subagents,
              reconcile: rec
            })
            return
          ...
          if (action === 'subagents') {
            ...
            const subagents = await listSubagentTree(ctx, rootSessionId)
            sendJson(res, 200, {
              ok: true,
              rootSessionId,
