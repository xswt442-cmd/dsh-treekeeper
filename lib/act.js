...
export function validateKillTarget({ pid, seenCreatedMs, facts, whitelistPids = new Set(), selfPid = process.pid, allowKill = true }) {
  ...
  // A failed query proves nothing: `alive: false` here can mean the process is
  // gone OR that PowerShell could not be run. Only a clean "not found" answer
  // may read as already gone; a broken one must refuse, or a transient failure
  // on a live target would be reported to the caller as a successful no-op.
  if (facts.error) return { ok: false, code: 'verify_failed', detail: facts.error }
 ...
 * @param {{alive:boolean, createdMs:number|null, error?:string}} after - probe taken after taskkill.
 * @returns {{ok:boolean, code:string, detail?:string}}
 */
export function classifyKillOutcome(beforeCreatedMs, after) {
  // A probe that errored cannot prove anything about the pid. Treating it as
  // death would report a kill that may not have happened — taskkill already
  // ran, so this is the one spot where a tooling failure must read as failure,
  // never as success.
  if (after.error) return { ok: false, code: 'verify_failed', detail: after.error }
