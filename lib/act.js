...
/**
 * Ownership gate for a kill target: the pid must be attributed into the DSH
 * host tree (a host descendant or a whitelisted root). A process outside that
 * tree — the `unknown` bucket the UI shows read-only — must never reach
 * taskkill, otherwise TreeKeeper could terminate unrelated applications. Pure:
 * no OS access, easy to unit test. `attribution` is the serialized
 * `lastSnapshot.attribution` object (pid -> info).
 */
export function validateKillOwnership(attribution, pid) {
  if (!attribution || !(pid in attribution)) return { ok: false, code: 'unattributed' }
