...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Left offset used when the DSH shell geometry cannot be measured (see
    // measureDockLeft). 80px ≈ collapsed sidebar + gutter: the smallest value
    // that kept the dock clear of the sidebar in every shell build seen so
    // far. Failure here is a misplaced dock, not a broken one, so the panel
    // stays usable — which is exactly why it must not fail silently.
    const DOCK_LEFT_FALLBACK_PX = 80

    // One warning per page, not per render: the dock keeps working when the
    // shell chain is missing, so a repeating log would only train people to
    // ignore it. Guarded because reduced DOM contexts have no console.
    const warnDockGeometry = (left) => {
      if (typeof console === 'undefined' || typeof console.warn !== 'function') return
      console.warn('[dsh-treekeeper] utility dock: DSH shell geometry unavailable ' +
        '([data-shell-overlay] → frame → sidebar); falling back to left=' + left + 'px')
    }
    ...
    function getUtilityDock() {
      ...
      // The dock container is ours (document.body.appendChild below); DSH's
      // slots supply panel content but no geometry, so the sidebar's right
      // edge has to be measured through the shell DOM. Any missing link
      // (renamed attribute, dock mounted before the shell, reduced DOM)
      // degrades to the fallback offset and says so once.
    ...
    const I18N = {
      zh: { title: 'TreeKeeper', subtitle: '进程树对账', leaks: '泄漏告警', none: '没有发现异常', jobs: '任务账本', subagents: '子代理树', selectSession: '选择一个会话后刷新', unavailable: '当前 DSH 未提供此能力', host: 'DSH 宿主后代', unknown: '未归属进程', kill: '树杀', confirm: '确认杀整棵树？', confirmBody: '将对以下进程树执行 taskkill /T /F：', refresh: '刷新', refreshing: '采样中', close: '关闭', sampling: '正在读取当前宿主的进程快照…', degraded: '降级采样（无父进程链，已禁止树杀）', unattr: '未归属', closed: '连接失败', findings: '发现', descendants: '后代', hard: '确证', inferred: '推断', inferredSection: '推断级发现（仅指示性线索，不可树杀）' },
      en: { title: 'TreeKeeper', subtitle: 'process reconciliation', leaks: 'Leak findings', none: 'Nothing unusual', jobs: 'Job ledger', subagents: 'Subagent tree', selectSession: 'Select a session, then refresh', unavailable: 'This DSH build does not provide the capability', host: 'DSH host descendants', unknown: 'Unattributed', kill: 'Kill tree', confirm: 'Kill the whole tree?', confirmBody: 'taskkill /T /F will run on this tree:', refresh: 'Refresh', refreshing: 'Sampling', close: 'Close', sampling: 'Reading the current host process snapshot…', degraded: 'Degraded sampling (tree kill disabled)', unattr: 'unattributed', closed: 'request failed', findings: 'Findings', descendants: 'Descendants', hard: 'hard', inferred: 'inferred', inferredSection: 'Inferred findings (indicative only, no kill)' }
    }
    ...
    function Panel(props) {
      const { t, data, error, loading, armed, onArm, onKill, onRefresh, onClose } = props
      ...
      return h('section', { className: 'tk-panel', 'aria-label': t.title },
        ...
        h('div', { className: 'tk-body' },
          ...
          loading && !data ? h('div', { className: 'tk-loading' }, t.sampling) : h('div', { className: 'tk-sec' },
            ...
            h('details', { className: 'tk-disclosure' },
              h('summary', null, t.subagents, h('span', { className: 'tk-badge tk-dim' }, String(subagents.length))),
              data && data.subagentAvailability === 'root-required'
                ? h('div', { className: 'tk-empty' }, t.selectSession)
                : data && data.subagentAvailability === 'unavailable'
    ...
    function findingRow(t, finding, index, armed, onArm, onKill, data, canKill) {
      ...
      return h('div', { className: 'tk-row', key: 'f' + (finding.key ?? index) },
        ...
        tier === 'hard' && canKill && finding.pids && finding.pids.length === 1
    ...
    function TreeKeeperSurface() {
      ...
      const refresh = async () => {
        setLoading(true)
        try {
          const current = sessionsService && sessionsService.list && typeof sessionsService.list.getSnapshot === 'function'
            ? sessionsService.list.getSnapshot().current
            : null
          const query = current ? '&rootSessionId=' + encodeURIComponent(current) : ''
          const res = await fetch('/dsh-treekeeper/api?action=snapshot' + query, { headers: { accept: 'application/json' } })
