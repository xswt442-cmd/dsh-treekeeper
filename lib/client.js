  ...
  factory: (require) => {
    ...
    function processRow(t, armed, onArm, onKill, process, canKill) {
      return h('div', { className: 'tk-row', key: 'p' + process.pid },
        ...
        // Only attributed host-tree processes (`evidence: 'exact'`) get a kill
        // entry: the `unknown` bucket is investigation-only, so it stays
        // read-only and never exposes a tree-kill button.
        canKill && process.evidence === 'exact' ? killBtn(t, armed, onArm, onKill, process.pid, process.cmdline || process.name, process.createdMs) : null)
    }
