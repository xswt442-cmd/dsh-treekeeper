...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const I18N = {
      zh: { title: 'TreeKeeper · 进程树对账', leaks: '泄漏告警', none: '没有发现异常', jobs: '任务账本', unknown: '未归属进程', os: 'OS 进程', kill: '树杀', confirm: '确认杀整棵树？', confirmBody: '将对以下进程树执行 taskkill /T /F：', refresh: '刷新', degraded: '降级采样（无父进程链，归属已停用）', unattr: '未归属', closed: '连接失败' },
      en: { title: 'TreeKeeper · process tree', leaks: 'Leak findings', none: 'Nothing unusual', jobs: 'Job ledger', unknown: 'Unattributed', os: 'OS process', kill: 'Kill tree', confirm: 'Kill the whole tree?', confirmBody: 'taskkill /T /F will run on this tree:', refresh: 'Refresh', degraded: 'Degraded sampling (no parent chain; attribution disabled)', unattr: 'unattributed', closed: 'request failed' }
    }
          ...
          h('div', { className: 'tk-sec' },
            ...
            findings.length === 0
              ...
              : findings.map((f, i) => h('div', { className: 'tk-row', key: 'f' + i },
                  ...
                  f.pids && f.pids.length === 1
                    ? killBtn(t, armed, onArm, onKill, f.pids[0], (f.evidence && f.evidence.sample) || f.detail)
                    : null)),
            h('div', { className: 'tk-sechead' }, t.unknown + ' · ' + unknown.length),
            unknown.length === 0
              ? h('div', { className: 'tk-empty' }, t.none)
              : unknown.slice(0, 40).map((p) => h('div', { className: 'tk-row', key: 'p' + p.pid },
                  h('span', { className: 'tk-badge tk-dim' }, 'pid ' + p.pid),
                  h('div', { style: { flex: 1, minWidth: 0 } },
                    h('div', { className: 'tk-cmd' }, (p.cmdline || p.name || '').slice(0, 200)),
                    h('div', { className: 'tk-dim' }, Math.round((p.wsBytes || 0) / 1048576) + ' MB' + (p.createdMs ? ' · ' + Math.max(0, Math.round((Date.now() - p.createdMs) / 60000)) + ' min' : ''))),
                  killBtn(t, armed, onArm, onKill, p.pid, p.cmdline || p.name, p.createdMs))),
            h('div', { className: 'tk-sechead' }, t.jobs + ' · ' + rows.filter((r) => r.source === 'job').length),
            rows.filter((r) => r.source === 'job').length === 0
              ? h('div', { className: 'tk-empty' }, t.none)
              : rows.filter((r) => r.source === 'job').map((r, i) => h('div', { className: 'tk-row', key: 'j' + i },
                  h('span', { className: 'tk-badge tk-ok' }, r.status),
                  h('div', { style: { flex: 1, minWidth: 0 } },
                    h('div', { className: 'tk-cmd' }, (r.label || '').slice(0, 160)),
                    h('div', { className: 'tk-dim' }, r.id + (r.pids && r.pids.length ? ' → pid ' + r.pids.join(', ') : ' → ' + (r.indicative ? 'no pid match (indicative join)' : ''))))))),
    ...
    return { unload: () => { const f = document.querySelector('.tk-fab'); if (f) f.remove(); if (rootHolder.el) rootHolder.el.remove() } }
  }
})
