...
export function createJobLedger(ctx) {
  ...
  if (typeof ctx.inject === 'function') {
    ctx.inject(['jobs', 'agents'], (scope) => {
      ...
      refresh()
      // The subscription is owned by the JOBS service's own context, not by
      // this scope (packages/jobs/jobs-local binds it through layers.effect on
      // its own ctx), so nothing detaches it when this fence goes away — every
      // reload left its listener behind, each still running the guarded
      // refresh on every registry change. Keep the disposer and use it.
      const detach = typeof scope.jobs.onJobsChanged === 'function'
        ? scope.jobs.onJobsChanged(refresh)
        : null
