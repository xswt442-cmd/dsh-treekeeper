...
export function createSubagentTree(ctx) {
  ...
  return {
    async list(rootSessionId) {
      ...
      return listSubagentTree(ctx, rootSessionId)
    }
  }
}
...
export async function listSubagentTree(ctx, rootSessionId) {
  ...
  try {
    ...
    return await callListDescendants(subagents.listDescendants.bind(subagents), rootSessionId)
  } catch {
    return null
  }
}

async function callListDescendants(listDescendants, rootSessionId) {
  try {
    return await listDescendants(rootSessionId)
  } catch {
    return null
  }
}
